...
class task1:
    def __init__(self):
        ...
        # this is only for RVIZ to visualize move_group commands, do we need this then? I think NO
        # self.pub2 = rospy.Publisher('/move_group/display_planned_path', moveit_msgs.msg.DisplayTrajectory)
        # rospy.sleep(10)
    ...
    def move_gripper(self, v):
        ...
        # self.gripper.clear_pose_targets() # it is supposed to be used with pose planner
    ...
    def callback_fn(self, s):
        ...
        if m[0] == 'bottle': # or m[0] == 'vase': # msg converter sends only this msgs but anyway check it 
            ...
            if int(m[2]) - int(m[1]) < CRITICAL_BOX_SIZE and self.moving == False:
                self.move('f')
            else:
                self.stop_robot()
                self.pick()
